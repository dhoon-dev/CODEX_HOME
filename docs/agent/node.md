...
- If `package.json` contains a `packageManager` field (e.g., `pnpm@9.0.0`), treat it as a strong signal for the
  intended package manager (and often its version). Prefer `corepack` when applicable.
...
- If multiple lockfiles exist or the intended package manager is unclear, follow CI/build scripts to resolve the
  intended toolchain (and call out any mismatch).
